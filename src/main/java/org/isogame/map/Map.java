...
public class Map {
    ...
    // ... (generateMap, calculateCombinedNoise, determineTileTypeFromElevation, populateSaveData, loadState, smoothTerrain, findSuitableCharacterPosition, getTile, isValid, getters - as in previous correct version)
    ...
    public boolean placeBlock(int r, int c, Item itemToPlace) {
        ...
        queueLightUpdateForArea(r,c,2,this.lightManager); // Update light for a slightly larger area
    ...
    public void populateSaveData(MapSaveData saveData) {
        ...
        for (int r = 0; r < this.height; r++) {
            ...
            for (int c = 0; c < this.width; c++) {
                ...
                } else { // Should not happen if map is fully initialized
    ...
    public void setTileElevation(int r, int c, int newElevation) {
        ...
        if (oldElevation != clampedElevation || oldType != tile.getType() || oldTorchState != tile.hasTorch()) {
            queueLightUpdateForArea(r, c, 2, this.lightManager); // Increased radius for more thorough update
        ...
        for (int dr = -radius; dr <= radius; dr++) {
            for (int dc = -radius; dc <= radius; dc++) {
                ...
                if (isValid(nr, nc)) {
                    ...
                    if (t != null) {
                        // For sky light, if it's exposed, it should try to get the current global target.
                        // Otherwise, if it had light, it needs removal.
                        if (lm.isSurfaceTileExposedToSky(nr, nc, t.getElevation())) {
                            // Add to propagation with current global target to ensure it (re)propagates this value
                        ...
                        // If it had a torch, ensure its light source is re-asserted
                        if (t.hasTorch()) {
    ...
    public void toggleTorch(int r, int c) {
        ...
        if (tile != null && tile.getType() != Tile.TileType.WATER && tile.isSolidOpaqueBlock()) {
            ...
            queueLightUpdateForArea(r, c, 2, lightManager); // Update area around torch
        }
    }
    ...
    public void propagateLightAcrossChunkBorder(LightManager.ChunkCoordinate sourceChunkCoord, LightManager.ChunkCoordinate targetChunkCoord, LightManager lm) {
        ...
        int sourceEndX = sourceStartX + CHUNK_SIZE_TILES;
        int sourceEndY = sourceStartY + CHUNK_SIZE_TILES;

        // Iterate over all tiles in the source chunk
        for (int r = sourceStartY; r < sourceEndY; r++) {
            for (int c = sourceStartX; c < sourceEndX; c++) {
                if (!isValid(r,c)) continue;

                // Check if this tile is on the border facing the target chunk
                boolean onBorder = false;
                if (targetChunkCoord.chunkX > sourceChunkCoord.chunkX && c == sourceEndX - 1) onBorder = true; // East border
                else if (targetChunkCoord.chunkX < sourceChunkCoord.chunkX && c == sourceStartX) onBorder = true; // West border
                else if (targetChunkCoord.chunkY > sourceChunkCoord.chunkY && r == sourceEndY - 1) onBorder = true; // South border
                else if (targetChunkCoord.chunkY < sourceChunkCoord.chunkY && r == sourceStartY) onBorder = true; // North border

                if (onBorder) {
                    Tile tile = getTile(r, c);
                    if (tile != null) {
                        // Queue this border tile for propagation. Its processing will attempt to light up neighbors in the target chunk.
                        if (tile.getSkyLightLevel() > 0) {
                            if (!lm.getSkyLightPropagationQueue_Direct().contains(new LightManager.LightNode(r,c,tile.getSkyLightLevel())))
                                lm.getSkyLightPropagationQueue_Direct().add(new LightManager.LightNode(r, c, tile.getSkyLightLevel()));
                        }
                        if (tile.getBlockLightLevel() > 0) {
                            if (!lm.getBlockLightPropagationQueue_Direct().contains(new LightManager.LightNode(r,c,tile.getBlockLightLevel())))
                                lm.getBlockLightPropagationQueue_Direct().add(new LightManager.LightNode(r, c, tile.getBlockLightLevel()));
                        }
                    }
                }
            }
